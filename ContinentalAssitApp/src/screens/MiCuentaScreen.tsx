...
import { Telefono, TelefonosRespuesta } from '../interfaces/Login';
...
export const MiCuentaScreen = () => {
  ...
  const { usuarioLogin, idioma } = useContext(AuthContext);
  ...
  const AlertEliminar = (telefono: Telefono) => {
    Alert.alert('Eliminar', '¿Está seguro de eliminar este teléfono?', [
      {
        text: 'Cancelar',
  ...
  const eliminarTelefono = async (telefono: Telefono) => {
    ...
    try {
      const resp = await continentalApi.post<Telefono>(
        '/app_eliminar_telefono',
        datosLogin,
        { headers },
      );
      ...
      if (resp.data) {
        Alert.alert(
          'Teléfono eliminado',
          'El teléfono se eliminó correctamente',
  ...
  const agregarTelefono = async (data: Telefono) => {
    ...
    try {
      const resp = await continentalApi.post<TelefonosRespuesta>(
        '/app_insertar_telefono',
        dataTelefono,
        { headers },
      );
      if (resp.data) {
        ...
        Alert.alert(
          'Teléfono agregado',
          'El teléfono se agregó correctamente',
          [
            {
              text: 'Ok',
              onPress: () => {
        ...
        <PaperProvider>
          <ScrollView>
            <View style={Style.imgContainer}>
              {usuarioLogin.avatar ? (
                <Image
                  source={{ uri: usuarioLogin.avatar }}
                  style={Style.imagen}
                />
              ) : (
                ...
                {telefonos.map((telefono, index) => (
                  <View key={index}>
                    <View style={Style.telContainer}>
                      ...
                      <View>
                        <TouchableOpacity
                          onPress={() => AlertEliminar(telefono)}>
                          <Text>
            ...
            <Portal>
              <Modal
                visible={visible}
                onDismiss={hideModal}
                contentContainerStyle={Style.modaContainer}>
                {phoneComponent}

                <View style={Style.telBotonContainer}>
                  <View style={Style.containerBotton}>
                    <TouchableOpacity
                      onPress={handleSubmit(agregarTelefono)}
