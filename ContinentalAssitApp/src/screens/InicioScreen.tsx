...
export const InicioScreen = ({ navigation }: Props) => {
  ...
  const { session, isGeolocation, logout, login } =
  ...
  const detectarInternet = async () => {
    ...
    if (stateInternet.isConnected === true) {
      ...
      Alert.alert(
        'La conexión a Internet ha fallado, algunas funciones podrían no estar disponibles',
      );
    }
  };
