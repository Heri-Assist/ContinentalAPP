...
export const ChatScreen = ({ navigation }: Props) => {
  ...
  const [welcomeMessage, setWelcomeMessage] = useState<MessageChat | null>(
    null,
  );
  ...
  const iniciarChat = async () => {
    ...
    setWelcomeMessage({
      mensaje:
        idioma === 'es'
          ? 'Bienvenido a Continental Assist, ¿En qué podemos ayudarte?'
  ...
  const inicializarChat = async () => {
    ...
    onChildAdded(mensajesRef, snapshot => {
      ...
      setListaMensajes(prevListaMensajes => [
        ...prevListaMensajes,
        mensajeEntrada,
      ]);
      setIsLoading(false);
    ...
    onChildAdded(dbRef, snapshot => {
      ...
      setListaMensajes(prevListaMensajes => [
        ...prevListaMensajes,
        { ...mensajeSalida, isSent: false },
      ]);
    });

    setListaMensajes(prevListaMensajes =>
      prevListaMensajes.sort((a: any, b: any) => a.fecha - b.fecha),
    );
  ...
  const handleAttachFile = async (file: any) => {
    Alert.alert('Adjuntar Imagen', '¿Deseas adjuntar una Imagen?', [
      ...
      {
        text: 'Cámara',
        onPress: () => launchCamera({ mediaType: 'photo' }, handleFileSelected),
      },
      {
        text: 'Galería',
        onPress: () =>
          launchImageLibrary({ mediaType: 'photo' }, handleFileSelected),
      },
    ]);
        ...
        <KeyboardAvoidingView
          ...
          <View style={Style.container}>
            <ScrollView contentContainerStyle={Style.messagesContainer}>
              {[welcomeMessage, ...listaMensajes.filter(Boolean)].map(
                (message, index) => {
                  let date;
                  if (
                    message &&
                    message.fecha &&
                    typeof message.fecha === 'number'
                  ) {
                    date = new Date(message.fecha);
                  } else if (
                    message &&
                    message.fecha &&
                    'seconds' in (message.fecha as object)
                  ) {
                    date = new Date(
                      (message.fecha as { seconds: number }).seconds * 1000,
                    );
                  } else {
                    date = new Date();
                  }
